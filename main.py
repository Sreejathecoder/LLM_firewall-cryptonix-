...
class InjectionDetector:
    ...
    def detect(self, text: str) -> float:
        ...
        # Combine scores (60% similarity, 40% rules)
        injection_risk = 0.4 * rule_score + 0.6 * similarity
...
class ThreatFusion:
    ...
    def fuse(self, features: Dict[str, Any]) -> float:
        """Combine all features into threat probability"""
        ...
        if self.model:
            ...
            threat_prob = self.model.predict([feature_vector])[0]
...
class PolicyEngine:
    def __init__(self):
        # Don't encode policies during init to avoid the error
    ...
    def _load_policies(self) -> List[Dict]:
        ...
        return [
            {
                "id": "P001",
                "name": "Block High-Risk Injection Attempts",
                "condition": {
...
class LLMFirewall:
    ...
    async def analyze(self, request: PromptRequest) -> AnalysisResult:
        ...
        injection_risk = self.injection_detector.detect(normalized['text'])
